...
@Component({
	selector: 'ms-button-secondary',
	standalone: true,
	imports: [CommonModule],
	templateUrl: './ms-button-secondary.html',
	styleUrls: ['./ms-button-secondary.scss'],
	encapsulation: ViewEncapsulation.None
})
export class MsButtonSecondary implements OnInit, OnDestroy {
	@Input() label: string = '';
	...
	buttonClasses = computed(() => {
		const classes = [
			'ms-button-secondary',
			`ms-button-secondary--surface-${this.surface}`,
			this.disabled ? 'is-disabled' : '',
			this._isPressed() ? 'is-pressed' : '',
			this._isFocused() ? 'is-focused' : '',
			this.device ? `ms-button-secondary--device-${this.device}` : ''
		].filter(Boolean);
		return classes.join(' ');
	});
